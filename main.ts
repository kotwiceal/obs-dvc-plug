import { App, Editor, MarkdownView, Notice, Plugin, PluginSettingTab, 
	Setting, TAbstractFile, TFile } from 'obsidian';
...
class DVC {
	...
	constructor(plug: Plugin) {
		this.plug = plug;
		this.cwd = plug.app.vault.adapter.basePath;
	...
	cli(command: string, argument: string, show: boolean = true): void {
		this.shell(`dvc ${command} ${argument}`, show);
	}

	clif(command: string, argument: TAbstractFile[] | TFile[], show: boolean = true): void {
		let arg: string = argument.map(file => `"${file.path}"`).join(" ");
	...
	add(arg: string, show: boolean = true): void {
		this.cli('add', arg);
		this.getFiles();
	}

	push(arg: string, show: boolean = true): void {
		this.cli('push', arg);
	}

	pull(arg: string, show: boolean = true): void {
		this.cli('pull', arg);
	}

	addf(arg: TAbstractFile[] | TFile[]): void {
		this.clif('add', arg);
	}

	pushf(arg: TAbstractFile[] | TFile[]): void {
		this.clif('push', arg);
	}

	pullf(arg: TAbstractFile[] | TFile[]): void {
		this.clif('pull', arg);
	}
					...
					{com: "pull", icon: "book-down"}]
						.map(element => {
							menu.addItem((item) => {
								...
								.onClick(async () => {
									this.dvc.clif(element.com, [file]);
								});
						});
				})
			})
					...
					{com: "pull", icon: "book-down"}]
						.map(element => {
							menu.addItem((item) => {
								...
								.setIcon(element.icon)
								.onClick(async () => {
									this.dvc.clif(element.com, files);
								});
						});
				})
			})
